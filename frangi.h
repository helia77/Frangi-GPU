...
void frangi2d(const cv::Mat& src, cv::Mat& J, cv::Mat& scale, cv::Mat& directions, float beta, float c, float start, float end, float step, bool Black, bool gpu);

//Helper function - run 2D Hessian filter with parameter sigma on src, save to Dxx, Dxy and Dyy. 
void frangi2d_hessian(const cv::Mat& src, cv::Mat& Dxx, cv::Mat& Dxy, cv::Mat& Dyy, float sigma, bool gpu);
